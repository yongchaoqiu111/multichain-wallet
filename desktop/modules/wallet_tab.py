...
class WalletManagementTab(QWidget):
    ...
    def import_by_mnemonic(self):
        ...
        if self.mnemonic_input.isVisible():
            ...
            print(f'[DEBUG Desktop] 原始输入 repr: {repr(self.mnemonic_input.text())}')
            print(f'[DEBUG Desktop] strip后 repr: {repr(mnemonic)}')
            print(f'[DEBUG Desktop] 单词数量: {len(mnemonic.split())}')
            print(f'[DEBUG Desktop] 助记词内容: [{mnemonic}]')
            ...
            if wallet_info.get('mnemonic'):
                ...
                for chain in chains:
                    try:
                        ...
                        print(f"[OK] {chain} 钱包创建成功: {chain_info['address']}")
                    except Exception as e:
                        print(f"[ERROR] {chain} 钱包创建失败: {e}")
