...
class MultiChainWallet:
    ...
    def _mnemonic_to_private_key(self, mnemonic_words, account_index=0):
        ...
        print(f'[DEBUG Desktop Core] 助记词 repr: {repr(mnemonic_words)}')
        print(f'[DEBUG Desktop Core] 种子 hex: {seed.hex()[:64]}')
        ...
        print(f'[DEBUG Desktop Core] 私钥: {private_key_hex}')
    ...
    def derive_address(self, account_index=0):
        ...
        if self.chain == "TRON":
            ...
            print(f'[DEBUG Desktop Core] 公钥 hex: {public_key.hex()[:64]}')
            ...
            print(f'[DEBUG Desktop Core] ripemd: {ripemd.hex()}')
            print(f'[DEBUG Desktop Core] 地址: {address}')
        else:
